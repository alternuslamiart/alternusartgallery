...
export function CategoryBar() {
  ...
  return (
    <div
      ref={barRef}
      className="hidden lg:block w-full border-b bg-background relative"
    >
              ...
              <Link
                ...
                className={`inline-flex items-center px-3 py-1.5 text-[13px] font-medium rounded-full transition-all whitespace-nowrap ${
                  activeDropdown === cat.name
                    ? "text-foreground bg-sand-100"
                    : "text-muted-foreground hover:text-foreground hover:bg-sand-50"
                }`}
              >
                {cat.name}
              </Link>
        ...
        (cat) =>
          ...
          activeDropdown === cat.name && (
            <div
              ...
              onMouseEnter={() => handleMouseEnter(cat.name)}
              onMouseLeave={handleMouseLeave}
              className="absolute left-0 right-0 top-full bg-background border-b border-border shadow-[0_8px_30px_rgba(0,0,0,0.07)] animate-in fade-in slide-in-from-top-1 duration-200 z-50"
            >
                ...
                <div className="flex gap-12">
                  ...
                  {cat.dropdown.columns.map((col) => (
                    <div key={col.title} className="min-w-[160px]">
                      <h3 className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground mb-4">
                        {col.title}
                      </h3>
                  ...
                  {cat.dropdown.promo && (
                    <div className="ml-auto">
                      <Link
                        ...
                        onClick={() => setActiveDropdown(null)}
                        className="group block w-[280px] rounded overflow-hidden bg-sand-50 hover:shadow-lg transition-all"
                      >
                        <div className="h-[140px] bg-gradient-to-br from-sand-200 to-sand-100 relative overflow-hidden">
                        ...
                        <div className="p-4">
                          ...
                          <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-foreground/70 group-hover:text-foreground transition-colors">
