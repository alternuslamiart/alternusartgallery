...
function FloorPlanViewport({ p }: { p: Props }) {
  ...
  return <section className="floor-plan-canvas relative min-h-0 overflow-hidden bg-[#fbfbfa] text-zinc-800">
    ...
    <div className={`${chat?"":"hidden"} crystal-ai-prompt absolute bottom-[80px] left-1/2 flex w-[488px] max-w-[calc(100%_-_32px)] -translate-x-1/2 rounded-[12px] bg-[#202020] px-2 py-1 shadow-[0_12px_28px_rgba(0,0,0,.28)]`}><textarea aria-label="AI floor-plan prompt" value={p.prompt} onChange={event => p.onPromptChange(event.target.value)} onKeyDown={event => {if((event.ctrlKey||event.metaKey)&&event.key==="Enter")parsePrompt()}} placeholder="What do you want to create? Describe your floor plan..." className="h-9 min-h-9 flex-1 resize-none bg-transparent px-2 py-2 text-[11px] outline-none"/><button disabled={!p.prompt.trim()} onClick={parsePrompt} className="grid h-9 w-9 place-items-center rounded-[9px] bg-[#1687f7] text-white"><Send size={16}/></button></div>
