...
import { ArrowRight, Camera, ChevronDown, CircleHelp, Download, DoorOpen, EyeOff, FolderOpen, Glasses, Globe2, Grid3X3, Hand, Lightbulb, Link2, LoaderCircle, MessageSquare, MousePointer2, Plus, Ruler, Share2, Sofa, SquareDashed, TextCursorInput, Trash2, X } from "lucide-react";
...
function CanvasScene({camera,color,roughness,metallic,grid,selected,face,drawings,measurement,transform}:{camera:Camera;color:string;roughness:number;metallic:number;grid:boolean;selected:boolean;face:number|null;drawings:Point[][];measurement:{a:Point;b:Point}|null;transform:Transform}) {
  ...
  useEffect(()=>{const node=canvas.current;if(!node)return;const draw=()=>{const rect=node.getBoundingClientRect(),ratio=devicePixelRatio,ctx=node.getContext("2d");if(!ctx)return;node.width=rect.width*ratio;node.height=rect.height*ratio;ctx.setTransform(ratio,0,0,ratio,0,0);ctx.fillStyle="#0a0a0a";ctx.fillRect(0,0,rect.width,rect.height);
