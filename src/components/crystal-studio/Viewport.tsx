...
function FloorPlanViewport({ p }: { p: Props }) {
  ...
  return <section className="floor-plan-canvas relative min-h-0 overflow-hidden bg-[#fbfbfa] text-zinc-800">
    ...
    <div className="absolute left-1/2 top-2 flex -translate-x-1/2 rounded-[10px] border border-[#343434] bg-[#202020]/95 p-1 shadow-[0_8px_20px_rgba(0,0,0,.18)]">{(["floor-plan","modeling"] as const).map(value => <button key={value} onClick={() => p.onStudioModeChange?.(value)} className={`rounded-[8px] border px-5 py-2 text-[11px] capitalize transition-colors ${p.studioMode===value?"border-[#4A90D9] bg-[#1687f7] text-white":"border-transparent text-zinc-500 hover:text-zinc-300"}`}>{value==="floor-plan"?"Floor plan":"Modeling"}</button>)}</div>
    ...
    <div className={`${chat?"":"hidden"} crystal-ai-prompt absolute bottom-[80px] left-1/2 w-[488px] max-w-[calc(100%_-_32px)] -translate-x-1/2 rounded-[12px] border border-[#1687f7] bg-[#202020] px-2 pb-2 pt-1 shadow-[0_12px_28px_rgba(0,0,0,.28)]`}><textarea aria-label="AI floor-plan prompt" value={p.prompt} onChange={event => p.onPromptChange(event.target.value)} onKeyDown={event => {if((event.ctrlKey||event.metaKey)&&event.key==="Enter")parsePrompt()}} placeholder="What do you want to create? Describe your floor plan..." className="h-[68px] w-full resize-none bg-transparent px-2 py-3 text-[11px] outline-none"/><div className="flex h-10 items-center gap-2"><label className="grid h-9 w-9 cursor-pointer place-items-center rounded-[9px] bg-[#303030] text-zinc-200" aria-label="Attach reference"><Link2 size={16}/><input type="file" accept="image/png,image/jpeg" className="sr-only"/></label><label className="relative flex h-9 items-center rounded-[9px] bg-[#303030] text-[11px] text-zinc-200"><select aria-label="AI mode" value={aiModel} onChange={event => setAiModel(event.target.value)} className="h-full appearance-none bg-transparent pl-3 pr-8 outline-none"><option>Precision Mode</option><option>Fast Concept</option></select><ChevronDown size={14} className="pointer-events-none absolute right-2 top-3"/></label><button disabled={!p.prompt.trim()} onClick={parsePrompt} className="ml-auto grid h-9 w-9 place-items-center rounded-[9px] bg-[#1687f7] text-white disabled:opacity-40"><Send size={16}/></button></div></div>
...
export function Viewport(p:Props){
  ...
  return <section ref={canvasRef as React.RefObject<HTMLElement>} className={`relative min-h-0 overflow-hidden bg-[#171717] ${toolMode==="object"||toolMode==="draw"?"cursor-crosshair":toolMode==="move"?"cursor-move":toolMode==="rotate"?"cursor-grab":"cursor-default"}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={()=>{objectCreate.current=null;drag.current=null;setDrawing(null)}} onContextMenu={e=>e.preventDefault()} onWheel={e=>{e.preventDefault();setCamera(c=>({...c,distance:Math.max(2.1,Math.min(42,c.distance*Math.exp(e.deltaY*.0012)))}))}} onDragOver={e=>e.preventDefault()} onDrop={e=>{const id=e.dataTransfer.getData("text/asset-id");if(id){p.onAssetDrop(id);setSelected(true)}}}>
    ...
    <div className="absolute left-1/2 top-2 flex -translate-x-1/2 rounded-[10px] border border-[#343434] bg-[#202020]/95 p-1 shadow-[0_8px_20px_rgba(0,0,0,.18)]">{(["floor-plan","modeling"] as const).map(x=><button key={x} onClick={()=>{setMode(x);p.onStudioModeChange?.(x);setChat(true)}} className={`rounded-[8px] border px-5 py-2 text-[11px] transition-colors ${mode===x?"border-[#4A90D9] bg-[#3b3b3b] text-white":"border-transparent text-zinc-500 hover:text-zinc-300"}`}>{x==="floor-plan"?"Floor plan":"Modeling"}</button>)}</div>
