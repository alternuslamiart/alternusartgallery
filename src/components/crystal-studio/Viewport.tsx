...
export function Viewport(props: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="crystal-view-cube absolute right-7 top-5 z-30 grid w-[92px] grid-cols-3 gap-1 rounded-[14px] border border-white/10 bg-[#232323]/95 p-2 shadow-2xl backdrop-blur" aria-label="View cube navigation">
      ...
      <div className="absolute right-9 top-[132px] z-20 h-12 w-12" aria-label="Camera axis indicator"><span className="absolute left-5 top-5 h-2 w-2 rounded-full border border-[#1687f7]"/><span className="absolute left-6 top-0 h-6 w-[2px] bg-[#e95a55]"/><span className="absolute left-0 top-6 h-[2px] w-6 bg-[#73dc50]"/><span className="absolute left-6 top-6 h-5 w-[2px] bg-[#1687f7]"/></div>
      <div className="absolute right-6 top-[196px] z-20 grid gap-2">{[{icon:Lightbulb,label:"Lighting"},{icon:Globe2,label:"World"},{icon:Camera,label:"Reset view"},{icon:Grid3X3,label:"Assets"},{icon:CircleHelp,label:"Help"}].map(({icon:Icon,label})=><button key={label} aria-label={label} onClick={label==="Reset view"?()=>setView("Perspective"):undefined} className="grid h-10 w-10 place-items-center rounded-[10px] bg-[#252525]/95 text-zinc-100 shadow-lg backdrop-blur hover:bg-[#353535]"><Icon size={20}/></button>)}</div>
