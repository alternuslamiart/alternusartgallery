...
import { ArrowRight, Camera, Check, ChevronDown, CircleHelp, Download, DoorOpen, EyeOff, FolderOpen, Glasses, Globe2, Grid3X3, Hand, Lightbulb, Link2, LoaderCircle, MessageSquare, MousePointer2, Pause, Play, Plus, Ruler, Share2, Sofa, SquareDashed, TextCursorInput, Trash2, X } from "lucide-react";
...
function CanvasScene({camera,color,roughness,metallic,grid,selected,face,drawings,measurement,transform}:{camera:Camera;color:string;roughness:number;metallic:number;grid:boolean;selected:boolean;face:number|null;drawings:Point[][];measurement:{a:Point;b:Point}|null;transform:Transform}) {
  ...
  useEffect(()=>{const node=canvas.current;if(!node)return;const draw=()=>{const rect=node.getBoundingClientRect(),ratio=devicePixelRatio,ctx=node.getContext("2d");if(!ctx)return;node.width=rect.width*ratio;node.height=rect.height*ratio;ctx.setTransform(ratio,0,0,ratio,0,0);ctx.fillStyle="#0B0D10";ctx.fillRect(0,0,rect.width,rect.height);
    ...
    if(grid){const minorStep=camera.distance>34?2:camera.distance>20?1:camera.distance>10?.5:.25;const majorStep=minorStep*5;const extent=Math.min(20,Math.max(2.5,(camera.distance-NEAR_PLANE)/(Math.SQRT2*Math.max(.22,Math.cos(camera.pitch)))*.9));const worldCorners:[number,number,number][]=[[-extent,0,-extent],[extent,0,-extent],[extent,0,extent],[-extent,0,extent]];const corners=worldCorners.map(point=>project(point,camera,rect));const visibleCorners=worldCorners.every(point=>cameraDepth(point,camera)>NEAR_PLANE);
      ctx.save();if(visibleCorners){ctx.beginPath();ctx.moveTo(corners[0].x,corners[0].y);corners.slice(1).forEach((corner)=>ctx.lineTo(corner.x,corner.y));ctx.closePath();ctx.fillStyle="rgba(23,25,34,.9)";ctx.fill();ctx.clip();}
      for(let n=-extent;n<=extent;n+=minorStep){const rounded=Math.round(n/minorStep)*minorStep,axis=Math.abs(rounded)<.001,major=Math.abs(Math.round(rounded/majorStep)*majorStep-rounded)<.001;const stroke=axis?"#D84B58":major?"rgba(91,108,132,.58)":"rgba(55,67,83,.46)";const width=axis?1.5:major?.9:.55;line([rounded,0,-extent],[rounded,0,extent],stroke,width);line([-extent,0,rounded],[extent,0,rounded],axis?"#4BBF78":major?"rgba(91,108,132,.58)":"rgba(55,67,83,.46)",width)}ctx.restore();
    }
...
function FloorPlanViewport({ p, isMobile }: { p: Props; isMobile: boolean }) {
  ...
  return <section className="floor-plan-canvas relative min-h-0 overflow-hidden bg-[#fbfbfa] text-zinc-800">
    ...
    <svg ref={svgRef} className="absolute inset-0 h-full w-full cursor-crosshair" onPointerDown={onDown} onPointerMove={onMove} onPointerUp={completeDrag} onPointerCancel={() => {setDrag(null);setPanning(null)}} onWheel={event => { event.preventDefault(); setView(current => ({...current,zoom:Math.max(24,Math.min(180,current.zoom * Math.exp(-event.deltaY * .001)))})); }} onContextMenu={event => event.preventDefault()}>
      ...
      <rect width="100%" height="100%" fill="#fbfbfa"/>{!isMobile && <rect width="100%" height="100%" fill="url(#floor-dots)"/>}
...
export function Viewport(p:Props){
  ...
  useEffect(() => {
    ...
    window.addEventListener("crystal:toggle-ai-chat", toggleChat);
    return () => window.removeEventListener("crystal:toggle-ai-chat", toggleChat);
  }, []);
  ...
  useEffect(()=>{if(!playing)return;const timer=window.setInterval(()=>p.onTransformChange({...p.transform,rotation:p.transform.rotation+0.7}),50);return()=>window.clearInterval(timer)},[playing,p.onTransformChange,p.transform]);
  ...
  if (p.studioMode === "floor-plan") return <FloorPlanViewport p={p} isMobile={isMobile}/>;
  return <section ref={canvasRef as React.RefObject<HTMLElement>} className={`relative min-h-0 overflow-hidden bg-[#171717] ${mobileToolsOpen ? "crystal-mobile-tools-open" : ""} ${toolMode==="object"||toolMode==="draw"?"cursor-crosshair":toolMode==="move"?"cursor-move":toolMode==="rotate"?"cursor-grab":"cursor-default"}`} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={()=>{objectCreate.current=null;drag.current=null;setDrawing(null)}} onContextMenu={e=>e.preventDefault()} onWheel={e=>{e.preventDefault();setCamera(c=>({...c,distance:Math.max(2.1,Math.min(42,c.distance*Math.exp(e.deltaY*.0012)))}))}} onDragOver={e=>e.preventDefault()} onDrop={e=>{const id=e.dataTransfer.getData("text/asset-id");if(id){p.onAssetDrop(id);setSelected(true)}}}>
    {mode === "images" ? showImagePreview && <div className="crystal-mobile-build-frame"><img src={referenceImage || "/Build.png"} alt={referenceImage ? "Uploaded reference" : "Generated Build preview"} /><div className="crystal-mobile-build-actions"><button type="button" aria-label="Share image" onClick={() => { const image = referenceImage || `${window.location.origin}/Build.png`; if (navigator.share) void navigator.share({ title: "Crystal image", url: image }); else void navigator.clipboard?.writeText(image); }}><Share2 size={17} /></button><a href={referenceImage || "/Build.png"} download="Build.png" aria-label="Download image"><Download size={18} /></a></div></div> : <><CanvasScene camera={camera} color={p.color} roughness={p.roughness} metallic={p.metallic} grid={grid && !isMobile} selected={selected} face={face} drawings={drawing?[...drawings,drawing]:drawings} measurement={measurement} transform={p.transform}/><div className="pointer-events-none absolute bottom-5 left-5 rounded-md bg-black/25 px-2 py-1 text-[10px] text-zinc-300">RMB / MMB: orbit · Shift + LMB: pan · Ctrl + MMB: dolly · Wheel: zoom</div></>}
    <div className="crystal-mobile-modebar" role="tablist" aria-label="Mobile studio mode">
    ...
    <div className={`crystal-mobile-model-tools ${mobileToolsOpen ? "is-visible" : ""}`} aria-label="Studio tools">
      ...
      <button type="button" aria-label="Texture modeling" className={`crystal-mobile-texture-trigger ${textureOpen ? "active" : ""}`} onClick={() => { setTextureOpen(value => !value); setMobileToolsOpen(true); setChat(true); }}><CubeFilled size={18} /></button>
    ...
    <div className="crystal-status-pill absolute left-1/2 top-4 z-20 -translate-x-1/2"><Check size={12} />{p.loading ? "Generating Crystal model..." : "Crystal workspace ready"}</div>
    ...
    {chat ? <div className="crystal-ai-prompt absolute bottom-[80px] left-1/2 w-[488px] max-w-[calc(100%_-_32px)] -translate-x-1/2 rounded-[12px] bg-[#202020] px-2 pb-2 pt-1 shadow-[0_12px_28px_rgba(0,0,0,.28)]"><div className="flex items-start gap-2"><textarea aria-label="AI model prompt" value={p.prompt} onChange={e=>{p.onPromptChange(e.target.value);setChat(true)}} onFocus={()=>setChat(true)} onKeyDown={e=>{if((e.ctrlKey||e.metaKey)&&e.key==="Enter")submit()}} enterKeyHint="send" placeholder={selected?"Ask for changes to this object":"What do you want to create?"} className="h-[68px] min-w-0 flex-1 resize-none bg-transparent px-2 py-3 text-[11px] outline-none"/><button type="button" aria-label="Hide AI chat" title="Hide AI chat" onClick={()=>setChat(false)} className="mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-[7px] text-zinc-400 hover:bg-[#303030] hover:text-white"><X size={15}/></button></div>{referenceImage&&<div className="crystal-mobile-reference"><img src={referenceImage} alt="Uploaded reference" /><button type="button" aria-label="Remove uploaded image" onClick={()=>setReferenceImage(null)}><X size={13}/></button></div>}{p.error&&<div className="flex justify-between text-xs text-red-200"><span>{p.error}</span><button onClick={p.onClearError}><X size={14}/></button></div>}<div className="flex h-10 items-center gap-2"><label className="grid h-9 w-9 cursor-pointer place-items-center rounded-[9px] bg-[#303030] text-zinc-200" aria-label="Attach reference"><Link2 size={16}/><input type="file" accept="image/png,image/jpeg" className="sr-only" onChange={e=>{const file=e.target.files?.[0];if(file){setReferenceImage(URL.createObjectURL(file));setShowImagePreview(false);setMobileToolsOpen(false);setChat(true)}e.currentTarget.value=""}}/></label><label className="relative flex h-9 items-center rounded-[9px] bg-[#303030] text-[12px]"><select value={model} onChange={e=>setModel(e.target.value)} className="h-full bg-transparent pl-4 pr-9 outline-none"><option>Precision Mode</option><option>Fast Concept</option></select><ChevronDown size={15} className="absolute right-3 top-2"/></label><button type="button" aria-label="Send AI request" disabled={p.loading} onClick={submit} className="ml-auto grid h-9 w-9 place-items-center rounded-[9px] bg-[#1687f7] transition hover:bg-[#2d9bff] disabled:cursor-not-allowed disabled:opacity-60">{p.loading?<LoaderCircle className="animate-spin" size={18}/>:<ArrowRight size={17}/>}</button></div></div> : <button type="button" aria-label="Show AI chat" title="Show AI chat" onClick={()=>setChat(true)} className="crystal-ai-chat-toggle absolute bottom-[80px] left-1/2 grid h-10 w-10 -translate-x-1/2 place-items-center rounded-[10px] border border-[#1687f7] bg-[#202020] text-[#1687f2] shadow-[0_8px_20px_rgba(0,0,0,.2)]"><MessageSquare size={17}/></button>}
    <div className="crystal-modeling-tools absolute bottom-[18px] left-1/2 flex h-[52px] w-[488px] max-w-[calc(100%_-_32px)] -translate-x-1/2 items-center justify-between rounded-[12px] border border-[#343434] bg-[#202020] px-2 shadow-[0_10px_24px_rgba(0,0,0,.2)]">
      <div className="crystal-modeling-tool-group">{modelingTools.slice(0,5).map(t=><IconButton key={t.id} icon={t.icon} label={t.label} active={p.activeTool===t.id} onClick={()=>{p.onToolChange(t.id);if(t.id==="focus")focus();if(t.id==="orbit")setPlaying(v=>!v);if(t.id==="select")setSelected(false)}}/>)}<IconButton icon={SparkleFilled} label="Crystal AI Assistant" active={p.activeTool==="ai"&&chat} onClick={()=>{p.onToolChange("ai");setChat(v=>!v)}}/>{modelingTools.slice(5).map(t=><IconButton key={t.id} icon={t.icon} label={t.label} active={p.activeTool===t.id} onClick={()=>p.onToolChange(t.id)}/>)}</div>
      <span className="crystal-modeling-tool-divider" aria-hidden="true" />
      <button type="button" className="crystal-modeling-action" aria-label="Export viewport" onClick={p.onSnapshot}><Download size={14} /><span>Export</span></button>
      <button type="button" className={`crystal-modeling-action crystal-modeling-play ${playing ? "is-active" : ""}`} aria-label={playing ? "Pause preview" : "Play preview"} aria-pressed={playing} onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={14} /> : <Play size={14} />}</button>
    </div>
  </section>;
}
