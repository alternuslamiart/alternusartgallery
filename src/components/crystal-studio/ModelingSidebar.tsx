...
export function ModelingSidebar({ renderer, quality, material, prompt, loading, uploadedImage, onPromptChange, onGenerate, onUpload, onQualityChange, onMaterialChange, onRendererChange, onCollapse }: Props) {
  ...
  return <aside className="crystal-modeling-sidebar crystal-reference-left" aria-label="Objects panel">
    ...
    <section className="crystal-modeling-properties"><h2>Properties</h2><label>Model Quality <select value={quality} onChange={event => onQualityChange(event.target.value)}><option>Draft</option><option>Standard</option><option>High</option><option>Ultra</option></select></label><label>Material <select value={material} onChange={event => onMaterialChange(event.target.value as MaterialName)}>{materials.map(item => <option key={item}>{item}</option>)}</select></label><h3>Topology</h3><div>{["Auto", "Low Poly", "Topology"].map(item => <button key={item} onClick={() => onQualityChange(item)}>{item}</button>)}</div><h3>Texture</h3><div>{["None", "Standard", "PBR"].map(item => <button key={item}>{item}</button>)}</div><h3>Advanced</h3>{["Separate Parts", "Image Enhancement", "Code execution"].map(item => <button key={item} className="crystal-property-toggle">{item}<i /></button>)}</section>
