...
export function CrystalStudio() {
  ...
  return (
    <div className={`crystal-studio crystal-studio-enter fixed inset-0 z-[90] grid overflow-hidden bg-[#191919] text-zinc-100 ${leftOpen ? "" : "crystal-left-closed"} ${rightOpen ? "" : "crystal-right-closed"} ${alternateTheme ? "crystal-alt-theme" : ""}`}>
      {mobileSplash && <div className="crystal-mobile-splash"><img src="/logo.png" alt="Crystal" /></div>}
      <header className="col-span-full flex h-[72px] items-center border-b border-[#303030] bg-[#0F0F0F] px-7">
        <div className="flex items-center gap-4"><img src="/logo.png" alt="Crystal" className="crystal-brand-logo"/><b className="text-[20px] tracking-[-0.02em]">Crystal</b><button aria-label="Back to projects" onClick={() => setDashboard(true)} className="ml-7 text-zinc-200"><Home size={20}/></button></div>
        <nav aria-label="Application menu" className="relative ml-9 flex items-center gap-10 text-[14px] text-zinc-300">{["File", "Edit", "Tools", "Help", "View"].map((item) => <button aria-expanded={openMenu===item} key={item} onClick={() => setOpenMenu(openMenu === item ? null : item)} className={`transition hover:text-white ${openMenu === item ? "text-white" : ""}`}>{item}</button>)}{openMenu && <div className="absolute left-0 top-9 z-50 w-52 rounded-xl border border-white/10 bg-[#272727] p-1.5 shadow-2xl"><div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">{openMenu}</div>{menuItems[openMenu].map(action => <button key={action} onClick={() => runMenuAction(action)} className="block w-full rounded-lg px-3 py-2 text-left text-[11px] text-zinc-200 hover:bg-[#3a3a3a]">{action}</button>)}</div>}</nav>
      ...
      <Viewport renderer={renderer} selectedAsset={selectedAsset} activeTool={activeTool} prompt={prompt} loading={loading} progress={progress} error={error} transform={transform} color={color} roughness={roughness} metallic={metallic} onToolChange={setActiveTool} onPromptChange={setPrompt} onGenerate={handleGenerate} onTransformChange={setTransform} onClearError={() => setError(null)} onAssetDrop={(id) => { setSelectedAssetId(id); setTransform({ x: 0, y: 0, rotation: -8, scale: 1 }); }} onColorChange={setColor} onOpenPricing={() => setMobilePricing(true)} onSnapshot={()=>handleExport("JPEG & PNG")} />
