  ...
  return (
    <div onClick={() => contextMenu && closeContextMenu()} onContextMenuCapture={handleContextMenu} className={`crystal-studio crystal-ui-kit crystal-studio-enter ${studioMode === "modeling" ? "crystal-modeling-mode" : ""} ${embedded ? "crystal-studio-embedded" : "fixed inset-0 z-[90]"} grid overflow-hidden bg-[#191919] text-zinc-100 ${leftOpen ? "" : "crystal-left-closed"} ${rightOpen ? "" : "crystal-right-closed"} ${alternateTheme ? "crystal-alt-theme" : ""}`}>
      ...
      {contextMenu && <div role="menu" onPointerDown={event => event.stopPropagation()} className="fixed z-[120] flex h-[256px] w-[220px] max-w-[calc(100vw-24px)] flex-col gap-1 rounded-[20px] border-[2px] border-[#303030] bg-[#242424] p-1 text-white shadow-2xl" style={{ left: contextMenu.x, top: contextMenu.y }}>
        <button role="menuitem" onClick={() => void runContextAction("copy")} className="flex h-9 min-h-9 w-full items-center justify-between rounded-[12px] bg-[#363636] px-4 text-sm font-medium hover:bg-[#414141]"><span>Copy</span><span>Ctrl + C</span></button>
        ...
        <div className="relative">
          <button role="menuitem" onMouseEnter={() => setContextSubmenu("plugins")} className="flex h-9 min-h-9 w-full items-center justify-between rounded-[12px] px-4 text-sm font-medium hover:bg-[#303030]"><span>Plugins</span><span>▹</span></button>
