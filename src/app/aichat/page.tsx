  ...
  return (
    ...
    >
      ...
      <main className="relative m-2 flex min-w-0 flex-1 flex-col overflow-hidden rounded-[8px] bg-[#1a1a1a]">
        ...
        <form onSubmit={sendMessage} className="absolute bottom-6 left-1/2 flex h-12 w-[calc(100%-32px)] max-w-[781px] -translate-x-1/2 items-center gap-2 rounded-2xl border border-[#333] bg-[#282828] p-2 shadow-2xl transition focus-within:border-blue-500/60 focus-within:ring-4 focus-within:ring-blue-500/10">
          <button type="button" aria-label="Attach file" onClick={() => { setToast("File attachments are available in chat."); window.setTimeout(() => setToast(null), 1800); }} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#3c3c3c] text-zinc-300 transition hover:bg-[#484848] hover:text-white active:scale-95"><Paperclip size={20} /></button>
          ...
          <button type="button" aria-label="Use microphone" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#3c3c3c] text-zinc-300 transition hover:bg-[#484848] hover:text-white active:scale-95"><Mic size={20} /></button>
          <button type="submit" aria-label="Send message" disabled={!input.trim() || isSending} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#3b82f6] text-white shadow-lg shadow-blue-500/20 transition-all hover:scale-105 hover:bg-[#2563eb] active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"><ArrowUp size={20} /></button>
        </form>
      </main>
