  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] font-sans text-[#f5f5f5]">
      ...
      <aside className="fixed bottom-0 left-0 top-[54px] z-20 hidden w-[272px] border-r border-white/[0.04] bg-[#101010] px-3 lg:block">
        ...
        <div className="mt-2 flex h-[34px] items-center gap-2 rounded-[7px] bg-[#181818] px-3 text-[11px] text-[#8a8a8a]">
          <Search size={13} />
          Search
        </div>
      ...
      <main className="min-h-screen overflow-y-auto px-5 pb-12 pt-[102px] lg:ml-[272px] lg:px-10">
        <div className="mx-auto max-w-[765px]">
          <section>
            ...
            <input id="project-name" value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="e.g Exterior ON" className="mt-2 h-10 w-full rounded-[8px] bg-[#181818] px-3.5 text-[11px] text-white outline-none placeholder:text-[#777] focus:bg-[#1d1d1d]" />
          </section>
          ...
          <section className="mt-[43px]">
            ...
            <div className="mt-3 grid grid-cols-2 gap-[10px] sm:grid-cols-5">
              {styles.map((style) => (
                <button key={style.name} type="button" onClick={() => setSelectedStyle(style.name)} className={`relative h-[145px] overflow-hidden rounded-[8px] text-left transition ${selectedStyle === style.name ? "ring-1 ring-[#6b9b62]" : "hover:ring-1 hover:ring-white/30"}`}>
                  <img src={style.image} alt="" className="h-full w-full object-cover" />
                  <span className="absolute inset-x-1 bottom-1 rounded-[5px] bg-black/65 px-2 py-2 text-center text-[11px] font-medium backdrop-blur-sm">{style.name}</span>
                </button>
              ))}
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-2 lg:grid-cols-4">
                  {["Exterior", "Interior", "Aerial", "Concept"].map((type) => (
                    <button key={type} type="button" onClick={() => setImageType(type)} className={`h-[90px] rounded-[7px] text-xs font-medium transition ${imageType === type ? "bg-[#242424] text-white" : "bg-[#181818] text-zinc-300 hover:bg-[#222]"}`}>{type}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="space-y-3">
                  <label className="flex h-10 w-full items-center justify-between rounded-[8px] bg-[#181818] px-3 text-[11px] text-zinc-400"><select value={room} onChange={(event) => setRoom(event.target.value)} className="w-full appearance-none bg-transparent outline-none"><option value="">e.g Living Room Kitchen...</option><option>Living Room</option><option>Kitchen</option><option>Bedroom</option></select><ChevronDown size={14} /></label>
                  <label className="flex h-10 w-full items-center gap-3 rounded-[8px] bg-[#181818] px-2 text-[11px] text-zinc-400"><span className="grid h-7 w-7 place-items-center rounded bg-[#dd155a] text-base font-bold text-white">A</span><select value={model} onChange={(event) => setModel(event.target.value)} className="min-w-0 flex-1 appearance-none bg-transparent outline-none"><option>Autodesk Revit Fusion AI</option><option>Crystal Architecture AI</option><option>Concept Render AI</option></select><ChevronDown size={14} /></label>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mt-6">
            ...
            <div className="mt-4 grid gap-7 md:grid-cols-2">
              <div>
                ...
                <label className="flex h-12 w-full cursor-pointer items-center gap-2 rounded-[7px] bg-[#181818] px-2 text-left text-[9px] text-zinc-400">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {["1:1", "16:9", "9:16", "4:3"].map((ratio) => <button key={ratio} type="button" onClick={() => setAspectRatio(ratio)} className={`h-[34px] rounded-[6px] text-[10px] ${aspectRatio === ratio ? "bg-[#2f6df6] text-white" : "bg-[#181818] text-zinc-300 hover:bg-[#232323]"}`}>{ratio}</button>)}
                </div>
              </div>
              <div>
                <p className="mb-3 text-[11px] text-[#a0a0a0]">Number of images</p>
                <div className="flex h-[34px] w-[130px] items-center justify-between rounded-[6px] bg-[#181818] px-1.5"><button type="button" onClick={() => setCount(Math.max(1, count - 1))} className="grid h-7 w-7 place-items-center rounded-[5px] bg-[#222]"><ChevronLeft size={14} /></button><span className="text-[11px]">{count}</span><button type="button" onClick={() => setCount(Math.min(4, count + 1))} className="grid h-7 w-7 place-items-center rounded-[5px] bg-[#222]"><ChevronRight size={14} /></button></div>
              </div>
              <div>
                <p className="mb-3 text-[11px] text-[#a0a0a0]">Quality</p>
                <div className="grid grid-cols-3 gap-2">{["Standard", "HD", "Ultra"].map((value) => <button key={value} type="button" onClick={() => setQuality(value)} className={`h-[34px] rounded-[6px] text-[10px] ${quality === value ? "bg-[#2f6df6] text-white" : "bg-[#181818] text-zinc-300 hover:bg-[#232323]"}`}>{value}</button>)}</div>
              </div>
            </div>
            <label className="mt-6 block text-[11px] text-[#a0a0a0]" htmlFor="instructions">Special instructions</label>
            <input id="instructions" placeholder="Include a swimming pool, modern furniture" className="mt-2 h-12 w-full rounded-[8px] bg-[#181818] px-3 text-[10px] text-white outline-none placeholder:text-[#888] focus:ring-1 focus:ring-[#3d72ee]" />
          </section>
