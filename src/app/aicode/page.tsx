  ...
  return (
    <main className="flex h-screen min-h-[620px] w-full min-w-0 flex-1 flex-col overflow-hidden bg-[#0b0c0f] font-roboto text-[#e7eaf0]">
      ...
      <div className="flex min-h-0 flex-1 flex-col md:flex-row">
        ...
        <section className={`${previewMode ? "flex h-full" : panelVisible("Code")} min-w-0 flex-1 flex-col bg-[#0e0f12]`}>
          {!previewMode ? <><div className="flex h-12 items-center justify-between border-b border-white/[0.08] px-3"><div className="flex h-full items-center gap-3 text-xs"><span className="flex h-full items-center gap-2 border-b-2 border-blue-400 text-zinc-200"><FileCode2 size={14} />{activeFile.split("/").pop()}<span className="text-amber-400">●</span></span><button className="text-zinc-600 hover:text-white"><Plus size={14} /></button></div><div className="flex items-center gap-2 text-zinc-500"><button aria-label="Copy code" onClick={() => void navigator.clipboard?.writeText(code)}><Copy size={14} /></button><button aria-label="Run code" className="hover:text-white"><Play size={14} /></button></div></div><div className="min-h-0 flex-1 overflow-auto bg-[#0e0f12] p-4 font-mono text-[11px] leading-5"><textarea value={code} onChange={(event) => setCode(event.target.value)} spellCheck={false} aria-label="Code editor" className="h-full min-h-[240px] w-full resize-none bg-transparent text-[#c9d1dc] outline-none" style={{ tabSize: 2 }} /></div></> : <div className="relative m-4 min-h-0 flex-1 overflow-hidden rounded-xl border border-white/[0.1] bg-[#151922] shadow-[0_12px_30px_rgba(0,0,0,.22)]"><canvas ref={previewCanvasRef} className="h-full min-h-[280px] w-full" aria-label="Crystal Studio 3D grid preview" /><div className="absolute bottom-3 left-3 rounded-md bg-black/30 px-2 py-1 text-[10px] text-zinc-400">Crystal Studio grid</div></div>}
        </section>
      </div>
