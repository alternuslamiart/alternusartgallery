    ...
    <main className="flex h-screen min-h-[620px] w-full min-w-0 flex-1 flex-col overflow-hidden bg-[#0b0c0f] font-roboto text-[#e7eaf0]">
      ...
      <div className="flex min-h-0 flex-1 flex-col md:flex-row">
        ...
        <section className={`${previewMode ? "flex" : panelVisible("Code")} min-w-0 flex-1 flex-col bg-[#0e0f12]`}>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-auto bg-[#0e0f12] p-4 md:grid-cols-2"><div className="min-h-[280px] rounded-xl border border-white/[0.08] bg-[#111216] p-3 font-mono text-[11px] leading-5"><div className="mb-2 flex items-center gap-2 text-zinc-400"><FileCode2 size={13} />{activeFile.split("/").pop()}<span className="text-amber-400">●</span></div><textarea value={code} onChange={(event) => setCode(event.target.value)} spellCheck={false} aria-label="Code editor" className="h-[calc(100%-28px)] min-h-[240px] w-full resize-none bg-transparent text-[#c9d1dc] outline-none" style={{ tabSize: 2 }} /></div><div className="relative min-h-[280px] overflow-hidden rounded-xl border border-white/[0.08] bg-[#151922]"><canvas ref={previewCanvasRef} className="h-full min-h-[280px] w-full" aria-label="Crystal Studio 3D grid preview" /><div className="absolute bottom-3 left-3 rounded-md bg-black/30 px-2 py-1 text-[10px] text-zinc-400">Crystal Studio grid</div></div></div>
        </section>
      </div>
