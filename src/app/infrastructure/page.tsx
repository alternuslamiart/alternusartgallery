...
export default function InfrastructurePage() {
  ...
  return <main className="flex h-screen w-full overflow-hidden bg-[#0a0a0a] text-zinc-100">
    <aside className="flex w-[250px] shrink-0 flex-col border-r border-white/[0.08] bg-[#111216] p-4"><div className="mb-7 flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#3188f4]"><Route size={18} /></span><div><b className="text-sm">Crystal</b><p className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">Infrastructure</p></div></div><p className="mb-3 text-[10px] uppercase tracking-[0.18em] text-zinc-600">Tools</p><div className="space-y-1">{toolItems.map(({ type, label, icon: Icon }) => <button key={type} onClick={() => setTool(type)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-xs transition ${tool === type ? "bg-[#1d3c68] text-[#9ac3ff]" : "text-zinc-400 hover:bg-white/[0.06] hover:text-white"}`}><Icon size={15} />{label}</button>)}</div><div className="mt-auto rounded-xl border border-white/[0.08] bg-[#17191e] p-3 text-[11px] text-zinc-500">Click the grid to place the active element.</div></aside>
    <section className="flex min-w-0 flex-1 flex-col"><header className="flex h-16 shrink-0 items-center gap-5 border-b border-white/[0.08] px-5"><b className="text-sm">Crystal</b><span className="text-zinc-600">/</span><span className="text-xs text-zinc-300">3D Studio / Infrastructure</span><nav className="ml-6 flex rounded-lg border border-white/[0.08] bg-[#17191e] p-1 text-xs"><a href="/crystal" className="px-3 py-1.5 text-zinc-500 hover:text-white">Floor plan</a><a href="/crystal" className="px-3 py-1.5 text-zinc-500 hover:text-white">Modeling</a><a href="/infrastructure" className="rounded-md bg-[#1d3c68] px-3 py-1.5 text-[#9ac3ff]">Infrastructure</a></nav><div className="ml-auto flex items-center gap-2"><div className="relative"><button onClick={() => setSpecialOpen((value) => !value)} className="flex items-center gap-2 rounded-lg border border-white/[0.12] px-3 py-2 text-xs hover:border-white/30">Special <ChevronDown size={13} /></button>{specialOpen && <div className="absolute right-0 top-11 z-20 w-48 rounded-xl border border-white/[0.1] bg-[#17191e] p-1 shadow-xl">{["Residential Street", "Main Avenue", "Pedestrian-only Zone"].map((preset) => <button key={preset} onClick={() => setSpecialOpen(false)} className="block w-full rounded-lg px-3 py-2 text-left text-xs text-zinc-400 hover:bg-white/[0.06] hover:text-white">{preset}</button>)}</div>}</div><button onClick={() => exportLayout("GeoJSON")} className="flex items-center gap-2 rounded-lg border border-white/[0.12] px-3 py-2 text-xs hover:border-white/30"><Download size={14} />Export</button></div></header><div className="relative min-h-0 flex-1"><Canvas camera={{ position: [9, 8, 10], fov: 45 }}><Scene tool={tool} elements={elements} selectedId={selectedId} onPlace={place} onSelect={select} /></Canvas></div></section>
    <aside className="w-[270px] shrink-0 overflow-y-auto border-l border-white/[0.08] bg-[#111216] p-4"><h2 className="mb-4 text-sm font-semibold">Statistics</h2><div className="space-y-2">{[["Total Street Length", `${roads * 100} m`], ["Sidewalk Width", "1.5 m"], ["Tree Count", `${trees}`], ["Trees per 100m ratio", `${trees}`], ["Green Coverage", `${Math.min(100, trees * 8)}%`], ["Parking Spots", "0"]].map(([label, value]) => <div key={label} className="rounded-xl border border-white/[0.08] bg-[#17191e] p-3"><p className="text-[10px] text-zinc-500">{label}</p><p className="mt-1 text-sm font-semibold text-zinc-200">{value}</p></div>)}</div><h2 className="mb-3 mt-6 text-sm font-semibold">Suggestions</h2><div className="space-y-2">{suggestions.map((suggestion) => <div key={suggestion} className="flex gap-2 rounded-xl border border-amber-400/20 bg-amber-400/[0.06] p-3 text-[11px] leading-4 text-amber-100"><AlertTriangle size={14} className="mt-0.5 shrink-0 text-amber-300" />{suggestion}</div>)}{suggestions.length === 0 && <div className="flex gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/[0.06] p-3 text-[11px] text-emerald-100"><Lightbulb size={14} />Layout looks balanced.</div>}</div></aside>
  </main>;
}
